...
Qualtrics.SurveyEngine.addOnReady(function() {
    ...
    function displayRevised(qid, basenum) {
        ...
        let wtp_first_row = Number(getBoundByRow(qid, 0, eff_value)) - Number(getBoundByRow(qid, 0, trad_value));
        let wtp_last_row = Number(getBoundByRow(qid, len-1, eff_value)) - Number(getBoundByRow(qid, len-1, trad_value));
        let wtp_min = Math.min(wtp_first_row, wtp_last_row);
        let wtp_max = Math.max(wtp_first_row, wtp_last_row);

        let lower_bound;
        let upper_bound;

        if (wtp_upper >= wtp_max) {
            if (iseffLeft()) {
                row_num = len - 1;
            } else {
                row_num = -1;
            }
        } else if (wtp_lower <= wtp_min) {
            if (iseffLeft()) {
                row_num = -1;
            } else {
                row_num = len - 1;
